...
def evaluate_single_result(result_file, labels):
...
def perform_correlation_analysis(results_df, metric_columns):
    ...
    for metric in metric_columns:
        ...
        # Save plot
        plt.savefig(f'correlation_plot_{metric}.png')
        plt.close()
...
def main():
    ...
    data_path = f'sample_processed_internal_test_50_v1.2.csv' if DATASET == 'InterTest' else f'sample_processed_v6.4(외부병원).csv'
    ...
    for file in tqdm(result_files, desc="Evaluating results"):
        metrics = evaluate_single_result(file, labels)
        if metrics:
    ...
    output_filename = f'correlation_analysis_{API}_{DATASET}_{timestamp}.xlsx'
