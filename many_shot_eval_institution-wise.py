...
def get_user_inputs():
    ...
    institution = input("Enter the institution to analyze (A/B/C/D/E): ").upper()
    ...
    if institution not in ['A', 'B', 'C', 'D', 'E']:
        raise ValueError("Invalid institution")
    
    return api, dataset, n_shot, institution

def load_and_filter_data(data_path, dataset, target_institution):
    """Load data and filter for specific institution"""
    ...
    if dataset == 'External':
        ...
        # Map institutions to their full names
        institution_map = {
            'A': 'A (n=30)',
            'B': 'B (n=29)',
            'C': 'C (n=30)',
        ...
        # Filter for target institution
        valid_indices = institution_data['Institution'] == institution_map[target_institution]
        data = data[valid_indices]
        
        print(f"\nFiltered for Institution {target_institution} samples.")
...
def evaluate_single_result(result_file, labels, dataset, target_institution):
...
def main():
    try:
        # Get user inputs
        API, DATASET, TARGET_N_SHOT, INSTITUTION = get_user_inputs()
    ...
    data = load_and_filter_data(data_path, DATASET, INSTITUTION)
    labels = data.iloc[:,4:]
    ...
    for file in tqdm(result_files, desc=f"Evaluating results"):
        metrics = evaluate_single_result(file, labels, DATASET, INSTITUTION)
        if metrics:
    ...
    # Get metric columns (excluding n_shot, seed, filename, and institution)
    metric_columns = [col for col in results_df.columns 
                     if col not in ['n_shot', 'seed', 'filename', 'institution']]
    
    # Add timestamp and institution to output filename
    timestamp = pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')
    output_filename = f'performance_summary_{API}_{DATASET}_shot{TARGET_N_SHOT}_inst{INSTITUTION}_{timestamp}.xlsx'
